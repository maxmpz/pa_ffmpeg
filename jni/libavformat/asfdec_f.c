...
#if !PAMP_CHANGES // Begin PAMP change
...
static int asf_parse_packet(AVFormatContext *s, AVIOContext *pb, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        if (asf->packet_size_left < FRAME_HEADER_SIZE ||
            ...
            av_assert0(ret >= 0); // PAMP change - use our customized av_assert0
            /* fail safe */
            avio_skip(pb, ret);
                ...
                if (asf_st->pkt.size != asf_st->ds_packet_size * asf_st->ds_span) {
                    ...
                    if (buf) {
                        ...
                        while (offset < asf_st->pkt.size) {
                            ...
                            av_assert0(offset + asf_st->ds_chunk_size <= asf_st->pkt.size); // PAMP change - use our customized av_assert0
                            av_assert0(idx + 1 <= asf_st->pkt.size / asf_st->ds_chunk_size); // PAMP change - use our customized av_assert0
...
static int asf_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        if ((ret = asf_get_packet(s, s->pb)) < 0)
            av_assert0(asf->packet_size_left < FRAME_HEADER_SIZE || // PAMP change - use our customized av_assert0
                   asf->packet_segments < 1);
        asf->packet_time_start = 0;
    }
}
