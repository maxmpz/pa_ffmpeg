...
static int cache_read(URLContext *h, unsigned char *buf, int size)
{
    ...
    DLOG("%s read size=%d", __FUNC__, size);
...
resolve_eof:
    ...
    if ((whence == SEEK_SET && pos >= c->logical_pos ||
        ...
        if (   (whence == SEEK_SET && c->read_ahead_limit >= pos - c->logical_pos)
        	...
        	DLOG("%s reading read_ahead_limit=%d whence=%d pos=%" PRId64 " logical_pos=%" PRId64, __FUNC__, c->read_ahead_limit, whence, pos, c->logical_pos);
...
static const AVOption options[] = {
    { "read_ahead_limit", NULL_IF_CONFIG_SMALL("Amount in bytes that may be read ahead when seeking isn't supported, -1 for unlimited"), OFFSET(read_ahead_limit), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, INT_MAX, D }, // Pamp change
    {NULL},
};
