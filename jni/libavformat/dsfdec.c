...
#if PAMP_CHANGES // Begin PAMP change: don't read last block as it sometimes conains trash for some reason
    if(dsf->data_end > st->codec->block_align) {
    	dsf->data_end -= st->codec->block_align;
    	dsf->data_size -= st->codec->block_align;
    }
