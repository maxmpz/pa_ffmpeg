// PAMP Changes: added TMPDIR support
/*
...
#undef free
    ...
    size_t len = strlen(prefix) + 12 + strlen(tmpdir); /* room for "/tmp/" and "XXXXXX\0" */
#else
...
#endif
    DLOG("%s trying 1 filename=%s", __func__, *filename);
    fd = mkstemp(*filename);
    DLOG("%s got fd=%d", __func__, fd);
#if defined(_WIN32) || defined (__ANDROID__) || defined(__DJGPP__)
    if (fd < 0) {

        snprintf(*filename, len, "./%sXXXXXX", prefix);
        DLOG("%s trying 2 filename=%s", __func__, *filename);
        fd = mkstemp(*filename);
    }
#endif
#endif
