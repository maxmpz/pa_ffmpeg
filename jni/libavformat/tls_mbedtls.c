...
#include <mbedtls/debug.h> // Pamp change
...
static void f_dbg(void * data, int level, const char * file_name, int line, const char * msg) {
	LOGE("%s:%d %s", file_name, line, msg);
}
...
static int tls_open(URLContext *h, const char *uri, int flags, AVDictionary **options)
{
    ...
    //mbedtls_ssl_conf_dbg(&tls_ctx->ssl_config, f_dbg, NULL); // MaxMP: DEBUG NOTE: requires mbedtls MBEDTLS_DEBUG_C
